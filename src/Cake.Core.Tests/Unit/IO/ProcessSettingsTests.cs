...
namespace Cake.Core.Tests.Unit.IO
{
    public sealed class ProcessSettingsTests
    {
        public sealed class ExtensionMethods
        {
            ...
            [InlineData(true, true)]
            [InlineData(false, false)]
            public void Should_Return_Settings_With_Correct_EnableRaisingEvents(bool value, bool expected)
            {
                var settings = new ProcessSettings().SetEnableRaisingEvents(value);

                Assert.Equal(expected, settings.EnableRaisingEvents);
            }
