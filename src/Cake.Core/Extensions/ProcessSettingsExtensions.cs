...
namespace Cake.Core
{
    ...
    public static class ProcessSettingsExtensions
    {
        ...
        /// <summary>
        /// Sets a value indicating whether to enable events to be emitted by the process.
        /// </summary>
        /// <param name="settings">The process settings.</param>
        /// <param name="enable">true if events should be emitted by the process; otherwise, false. The default is false.</param>
        /// <returns>The same <see cref="ProcessSettings"/> instance so that multiple calls can be chained.</returns>
        public static ProcessSettings SetEnableRaisingEvents(this ProcessSettings settings, bool enable)
        {
            if (settings == null)
            {
                throw new ArgumentNullException("settings");
            ...
            settings.EnableRaisingEvents = enable;
            return settings;
        }
